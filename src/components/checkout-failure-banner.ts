...
export function showCheckoutFailureBanner(rawStatus: string): void {
  ...
  const retryButton = hasRetryTarget
    ? `<button id="cf-retry-btn" style="background:var(--bg);color:var(--status-alert);border:none;border-radius:4px;padding:4px 12px;font-weight:600;font-size:12px;cursor:pointer;white-space:nowrap;">${t('components.checkoutFailureBanner.retry', { defaultValue: 'Try again' })}</button>`
    : '';

  setTrustedHtml(banner, trustedHtml(`
    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--bg)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0;" aria-hidden="true">
    ...
    ${retryButton}
    <button id="cf-dismiss-btn" aria-label="${t('components.checkoutFailureBanner.dismiss', { defaultValue: 'Dismiss' })}" style="background:transparent;color:var(--bg);border:none;cursor:pointer;font-size:18px;padding:0 4px;line-height:1;">&times;</button>
