...
export function renderMilitaryVesselTooltipHtml(
  ...
  const sourceLabel = vessel.usniSource
    ? `<br/><span style="color:var(--status-watch);font-weight:600">${text(translate('popups.militaryVessel.estPosition'))}</span><br/><span style="opacity:.7">${text(translate('popups.militaryVessel.approximatePosition'))}</span>`
    : '';
