...
function makePreflight(origin) {
  return new Request('https://thepublicdispatch.com/api/health?compact=1', {
    method: 'OPTIONS',
    headers: {
      origin,
...
test('health preflight is compatible with credentialed browser fetches', async () => {
  const resp = await handler(makePreflight('https://www.thepublicdispatch.com'));

  assert.equal(resp.status, 204);
  assert.equal(resp.headers.get('access-control-allow-origin'), 'https://www.thepublicdispatch.com');
...
test('health GET response is compatible with credentialed browser fetches', async () => {
  const resp = await handler(new Request('https://thepublicdispatch.com/api/health?compact=1', {
    method: 'GET',
    headers: {
      origin: 'https://www.thepublicdispatch.com',
    },
  }));
