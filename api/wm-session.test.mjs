...
function makeReq(method, { origin, referer } = {}) {
  ...
  return new Request('https://thepublicdispatch.com/api/wm-session', { method, headers });
}
...
function finalCookieJar(cookies) {
  ...
  for (const cookie of cookies) {
    ...
    const domain = domainAttr ? domainAttr.slice('domain='.length).toLowerCase() : 'thepublicdispatch.com';
...
test('POST from trusted origin sets a valid HttpOnly wms_ session cookie without exposing token JSON', async () => {
  const resp = await handler(makeReq('POST', { origin: 'https://thepublicdispatch.com' }));
  ...
  assert.match(cookies.join('\n'), /wm-session=.*Domain=\.thepublicdispatch\.com/);
});
...
test('POST emits one anonymous mint usage event without exposing cookie material', async () => {
  ...
  const request = makeReq('POST', {
    origin: 'https://thepublicdispatch.com',
    referer: 'https://thepublicdispatch.com/reset-password?token=must-not-be-logged#also-not-logged',
  });
  ...
  assert.deepEqual(
    ...
    },
    {
      ...
      status: 200,
      auth_kind: 'anon',
      origin_kind: 'browser-same-origin',
      ip: null,
      referer: 'https://thepublicdispatch.com/reset-password',
      reason: 'ok',
    },
  );
...
test('session usage telemetry records verified Cloudflare client attribution and rejects forged headers', async () => {
  ...
  const verified = makeReq('POST', { origin: 'https://thepublicdispatch.com' });
  ...
  const forged = makeReq('POST', { origin: 'https://thepublicdispatch.com' });
  ...
  const tor = makeReq('POST', { origin: 'https://thepublicdispatch.com' });
...
test('OPTIONS preflight returns 204 with CORS', async () => {
  ...
  __resetRateLimitForTest();
  const resp = await handler(makeReq('OPTIONS', { origin: 'https://thepublicdispatch.com' }));
...
test('GET method is rejected with 405', async () => {
  const resp = await handler(makeReq('GET', { origin: 'https://thepublicdispatch.com' }));
...
test('failed mint outcomes emit their terminal status', async () => {
  ...
  const resp = await handler(makeReq('POST', { origin: 'https://thepublicdispatch.com' }), ctx);
  assert.equal(resp.status, 429);
  await settle();
...
test('telemetry stops delivery attempts when the Axiom sink is repeatedly unavailable', async () => {
  ...
  for (let index = 0; index < 20; index += 1) {
    const { ctx, settle } = makeWaitUntilCtx();
    const response = await handler(makeReq('POST', { origin: 'https://thepublicdispatch.com' }), ctx);
...
test('telemetry probes and closes the circuit after the outage window elapses', async () => {
  ...
  try {
    for (let index = 0; index < 20; index += 1) {
      const { ctx, settle } = makeWaitUntilCtx();
      await handler(makeReq('POST', { origin: 'https://thepublicdispatch.com' }), ctx);
      await settle();
    }
    now += 5 * 60 * 1000 + 1;
    axiomAvailable = true;
    ...
    await handler(makeReq('POST', { origin: 'https://thepublicdispatch.com' }), recovered.ctx);
    ...
    await handler(makeReq('POST', { origin: 'https://thepublicdispatch.com' }), resumed.ctx);
...
test('legacy widget/pro keys are moved into short-lived HttpOnly cookies', async () => {
  ...
  assert.match(joined, /wm-widget-key=widget-secret;.*Domain=\.thepublicdispatch\.com/);
  assert.match(joined, /wm-pro-key=pro-secret;.*Domain=\.thepublicdispatch\.com/);
...
test('legacy widget/pro secret checks reject prefix and length mismatches', async () => {
  ...
  try {
    const accepted = await handler(new Request('https://thepublicdispatch.com/api/wm-session', {
      method: 'POST',
      headers: {
        origin: 'https://thepublicdispatch.com',
        'content-type': 'application/json',
    ...
    const prefixOnly = await handler(new Request('https://thepublicdispatch.com/api/wm-session', {
      method: 'POST',
      headers: {
        origin: 'https://thepublicdispatch.com',
        'content-type': 'application/json',
    ...
    const differentLength = await handler(new Request('https://thepublicdispatch.com/api/wm-session', {
      method: 'POST',
      headers: {
        origin: 'https://thepublicdispatch.com',
        'content-type': 'application/json',
...
test('legacy key length boundary: 512 accepted, 513 rejected', async () => {
  ...
  try {
    ...
    const rejected = await handler(new Request('https://thepublicdispatch.com/api/wm-session', {
      method: 'POST',
      headers: {
        origin: 'https://thepublicdispatch.com',
        'content-type': 'application/json',
...
test('legacy cookie tombstones do not delete replacement HttpOnly key cookies', async () => {
  ...
  assert.equal(jar.get('wm-widget-key;.thepublicdispatch.com;/'), 'widget-secret');
  assert.equal(jar.get('wm-pro-key;.thepublicdispatch.com;/'), 'pro-secret');
});
