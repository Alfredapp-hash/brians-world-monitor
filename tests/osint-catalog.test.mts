...
describe('OSINT catalog fixture', () => {
  ...
  it('lives next to the types file, not as public/osint/catalog.json', () => {
...
describe('OSINT catalog loader', () => {
  it('fetches /osint/catalog.json and validates v2 shape', async () => {
    const loaded = await loadOsintCatalog(async (input) => {
      assert.equal(input, OSINT_CATALOG_URL);
      return {
        ok: true,
        status: 200,
        json: async () => catalog,
  ...
  it('does not invent tools when the catalog file is missing', async () => {
    await assert.rejects(
      () => loadOsintCatalog(async () => ({
        ok: false,
        status: 404,
        json: async () => ({}),
      })),
      (err: unknown) => {
        ...
        assert.match(err.message, /WAITING ON catalog\.json commit/);
        return true;
      },
    );
  });
