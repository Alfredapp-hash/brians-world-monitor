...
describe('embed public data auth', () => {
  ...
  it('lets anonymous iframe requests reach embed RPC handlers while nearby RPCs stay gated', async () => {
    ...
    for (const path of EMBED_PUBLIC_RPC_PATHS) {
      const res = await gateway(new Request(`https://brians-world-monitor.vercel.app${path}`, {
    ...
    const gated = await gateway(new Request('https://brians-world-monitor.vercel.app/api/conflict/v1/list-ucdp-events', {
      headers: { Origin: 'https://brians-world-monitor.vercel.app' },
    }));
  ...
  it('scopes anonymous bootstrap access to the weather embed key only', async () => {
    const publicReq = new Request('https://brians-world-monitor.vercel.app/api/bootstrap?keys=weatherAlerts', {
      headers: { Origin: 'https://brians-world-monitor.vercel.app' },
    });
    ...
    const rejected = [
      'https://brians-world-monitor.vercel.app/api/bootstrap',
      'https://brians-world-monitor.vercel.app/api/bootstrap?tier=fast',
      'https://brians-world-monitor.vercel.app/api/bootstrap?keys=weatherAlerts,marketQuotes',
      'https://brians-world-monitor.vercel.app/api/bootstrap?keys=marketQuotes',
      'https://brians-world-monitor.vercel.app/api/bootstrap?keys=weatherAlerts&debug=1',
      'https://brians-world-monitor.vercel.app/api/bootstrap?keys=weatherAlerts&keys=marketQuotes',
    ];

    for (const url of rejected) {
      const req = new Request(url, { headers: { Origin: 'https://brians-world-monitor.vercel.app' } });
