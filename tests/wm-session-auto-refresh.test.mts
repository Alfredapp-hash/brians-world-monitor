...
describe('wm-session refresh-on-401 (Layer 2)', () => {
  it('retries an API 401 with a freshly-minted token', async () => {
    ...
    const resp = await wrappedFetch('https://brians-world-monitor.vercel.app/api/bootstrap');
  ...
  it('does NOT retry when the path is in PREMIUM_RPC_PATHS', async () => {
    ...
    const resp = await wrappedFetch('https://brians-world-monitor.vercel.app/api/market/v1/analyze-stock');
  ...
  it('suppresses later anonymous API calls when a refreshed session is still rejected', async () => {
    ...
    try {
      ...
      const suppressed = await wrappedFetch('https://brians-world-monitor.vercel.app/api/infrastructure/v1/list-service-statuses');
  ...
  it('forwards only explicit credential-less public tier reads during the dead-session cooldown', async () => {
    ...
    console.warn = () => {};
    try {
      const failed = await wrappedFetch('https://brians-world-monitor.vercel.app/api/infrastructure/v1/list-service-statuses');
      assert.equal(failed.status, 401, 'failed recovery should enter the dead-session cooldown');

      const fast = await wrappedFetch('https://brians-world-monitor.vercel.app/api/bootstrap?tier=fast&public=1', {
        credentials: 'omit',
      });
      assert.equal(fast.status, 200, 'string input should reach the public tier while the session is dead');

      const slowRequest = new Request('https://brians-world-monitor.vercel.app/api/bootstrap?public=1&tier=slow', {
        credentials: 'omit',
      });
      ...
      const digest = await wrappedFetch('https://brians-world-monitor.vercel.app/api/news/v1/list-feed-digest?variant=full&lang=en&public=1', {
        credentials: 'omit',
      });
      assert.equal(digest.status, 200, 'public digest should bypass dead-session suppression');

      const displacement = await wrappedFetch('https://brians-world-monitor.vercel.app/api/displacement/v1/get-displacement-summary?flow_limit=50&public=1', {
        credentials: 'omit',
      });
      assert.equal(displacement.status, 200, 'public displacement should bypass dead-session suppression');

      const missingPublicFlag = await wrappedFetch('https://brians-world-monitor.vercel.app/api/bootstrap?tier=fast', {
        credentials: 'omit',
      });
      assert.equal(missingPublicFlag.status, 503, 'ordinary tier reads must remain session-gated');

      const credentialed = await wrappedFetch('https://brians-world-monitor.vercel.app/api/bootstrap?tier=fast&public=1', {
        credentials: 'include',
      });
    ...
    assert.deepEqual(
      forwarded.slice(-4),
      [
        { url: 'https://brians-world-monitor.vercel.app/api/bootstrap?tier=fast&public=1', credentials: 'omit' },
        { url: 'https://brians-world-monitor.vercel.app/api/bootstrap?public=1&tier=slow', credentials: 'omit' },
        { url: 'https://brians-world-monitor.vercel.app/api/news/v1/list-feed-digest?variant=full&lang=en&public=1', credentials: 'omit' },
        { url: 'https://brians-world-monitor.vercel.app/api/displacement/v1/get-displacement-summary?flow_limit=50&public=1', credentials: 'omit' },
      ],
  ...
  it('captures ONE wm_session_dead Sentry warning per degraded episode, not one per suppressed call', async () => {
    ...
    try {
      ...
      await wrappedFetch('https://brians-world-monitor.vercel.app/api/bootstrap');
      // Later calls are suppressed by the cooldown — no additional captures.
      const s1 = await wrappedFetch('https://brians-world-monitor.vercel.app/api/economic/v1/get-bls-series');
      const s2 = await wrappedFetch('https://brians-world-monitor.vercel.app/api/supply-chain/v1/get-shipping-stress');
  ...
  it('single-flights concurrent 401 recovery so only one retry verifies the mint', async () => {
    ...
    try {
      const responses = await Promise.all([
        wrappedFetch('https://brians-world-monitor.vercel.app/api/bootstrap'),
        wrappedFetch('https://brians-world-monitor.vercel.app/api/infrastructure/v1/list-service-statuses'),
        wrappedFetch('https://brians-world-monitor.vercel.app/api/infrastructure/v1/get-cable-health'),
      ]);
  ...
  it('replays a delayed stale 401 after another caller has refreshed the session', async () => {
    ...
    const first = wrappedFetch('https://brians-world-monitor.vercel.app/api/bootstrap');
    const delayed = wrappedFetch('https://brians-world-monitor.vercel.app/api/infrastructure/v1/get-cable-health');
