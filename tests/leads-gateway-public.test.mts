...
describe('leads gateway public access', () => {
  ...
  it('accepts an anonymous submit-contact POST (no API key, no session token)', async () => {
    ...
    const res = await gateway(new Request('https://brians-world-monitor.vercel.app/api/leads/v1/submit-contact', {
      method: 'POST',
      headers: {
  ...
  it('accepts an anonymous register-interest POST (no API key, no session token)', async () => {
    ...
    const res = await gateway(new Request('https://brians-world-monitor.vercel.app/api/leads/v1/register-interest', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Origin: 'https://brians-world-monitor.vercel.app',
