...
async function requestPublicRoute(origin: string) {
  ...
  return handler(new Request('https://brians-world-monitor.vercel.app/api/market/v1/list-market-quotes?symbols=AAPL', {
...
describe('gateway CDN origin policy', () => {
  it('keeps per-origin CORS without shared CDN caching for session-bearing first-party GETs', async () => {
    const res = await requestPublicRoute('https://brians-world-monitor.vercel.app');
    assert.equal(res.status, 200);
    assert.equal(res.headers.get('Access-Control-Allow-Origin'), 'https://brians-world-monitor.vercel.app');
  ...
  it('keeps per-origin CORS without shared CDN caching for session-bearing production subdomain GETs', async () => {
    const res = await requestPublicRoute('https://brians-world-monitor-git-preview.vercel.app');
    assert.equal(res.status, 200);
    assert.equal(res.headers.get('Access-Control-Allow-Origin'), 'https://brians-world-monitor-git-preview.vercel.app');
    assert.equal(res.headers.get('Vary'), 'Origin');
    assertNoSharedCacheHeaders(res);
  });

  it('avoids shared CDN caching for session-bearing preview origin GETs', async () => {
    const origin = 'https://brians-world-monitor-git-feature.vercel.app';
  ...
  it('preserves CDN caching for explicit anonymous public no-auth GETs', async () => {
    const origin = 'https://brians-world-monitor.vercel.app';
    const handler = createHandler();
    const res = await handler(new Request('https://brians-world-monitor.vercel.app/api/conflict/v1/list-acled-events', {
  ...
  for (const [path, rpc] of [
    ...
    it(`CDN-shields the public RPC variant when the router echoes ?rpc=: ${path}`, async () => {
      const handler = createHandler();
      const res = await handler(new Request(`https://brians-world-monitor.vercel.app${path}&rpc=${rpc}`, {
        headers: { Origin: 'https://brians-world-monitor.vercel.app' },
      }));
  ...
  it('skips CDN caching for degraded dataAvailable=false 200 responses', async () => {
    ...
    const res = await handler(new Request('https://brians-world-monitor.vercel.app/api/conflict/v1/list-acled-events?_debug=1', {
      headers: { Origin: origin },
    }));
  ...
  it('strips handler-supplied shared CDN headers on credential-bearing GETs', async () => {
    ...
    const res = await handler(new Request('https://brians-world-monitor.vercel.app/api/market/v1/list-market-quotes?symbols=AAPL', {
      headers: { Origin: 'https://brians-world-monitor.vercel.app', 'X-WorldMonitor-Key': sessionToken },
    }));
  ...
  it('preserves premium auth behavior', async () => {
    ...
    const noCreds = await handler(new Request('https://brians-world-monitor.vercel.app/api/market/v1/analyze-stock?symbol=AAPL', {
      headers: { Origin: 'https://brians-world-monitor.vercel.app' },
    }));
    assert.equal(noCreds.status, 401);
    assert.equal(noCreds.headers.get('Cache-Control'), 'no-store');

    const withKey = await handler(new Request('https://brians-world-monitor.vercel.app/api/market/v1/analyze-stock?symbol=AAPL', {
      headers: {
        Origin: 'https://brians-world-monitor.vercel.app',
        'X-WorldMonitor-Key': 'real-key-123',
      },
    ...
    assert.equal(withKey.headers.get('Access-Control-Allow-Origin'), 'https://brians-world-monitor.vercel.app');
