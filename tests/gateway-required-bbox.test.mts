...
function makeRequest(pathAndQuery: string): Request {
  ...
  return new Request(
    'https://brians-world-monitor.vercel.app' + pathAndQuery,
    { headers: { Origin: 'https://brians-world-monitor.vercel.app', 'X-WorldMonitor-Key': TEST_KEY } },
  );
}
...
function makeJsonPostRequest(path: string, bodyObject: Record<string, unknown>): Request {
  ...
  return new Request('https://brians-world-monitor.vercel.app' + path, {
    method: 'POST',
    headers: {
...
async function getGeneratedMilitaryFlightsPathAndQuery(req: Parameters<MilitaryServiceClient['listMilitaryFlights']>[0]): Promise<string> {
  let requestedUrl = '';
  const client = new MilitaryServiceClient('https://brians-world-monitor.vercel.app', {
    fetch: async (input) => {
