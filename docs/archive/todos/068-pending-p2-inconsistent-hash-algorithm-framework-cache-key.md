...
- undefined: Skipped (left pending) — standardizing on `sha256Hex` would require making `buildSummaryCacheKey()` in `src/utils/summary-cache-key.ts` async (Web Crypto `crypto.subtle.digest` has no sync form), which ripples into its call sites in `src/services/summarization.ts` (both call sites call it synchronously, unawaited, at lines ~267 and ~302). That file is outside this workstream's ownership list, so the change cannot be made safely/surgically here. Standardizing the other direction (FNV-1a in the intel handlers) would instead make `deduct-situation.ts`/`get-country-intel-brief.ts` internally inconsistent (they use `sha256Hex` for every other cache-key segment — query/context hashes — not just the framework one) and would weaken collision resistance for those attacker-controlled-text segments, which `server/_shared/hash.ts`'s own doc comment warns against. Recommend a follow-up task scoped to include `src/services/summarization.ts` so both call sites can be updated to `await` the new async signature together.
